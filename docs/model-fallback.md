...
Policy model IDs are not evidence by themselves. Recovery validates every policy chain against the selected `.opencode/agents/<agent>.md` frontmatter in the Task worktree and, when Main launches recovery, against Main's executable agent definitions as well. A missing definition or model mismatch is BLOCKED before routing state is created or evidence is recorded.
